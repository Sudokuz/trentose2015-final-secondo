   ...
   analyse : function (phrase){
       ...
       for (var i; i< patterns.length; i++){
           ...
           if(phrase.search(patterns[i].word) =! -1){             
                ...
                if(phrase.search(patterns[i].neutraliser) == -1 || phrase.search(patterns[i].support =! -1){
...
var FlirtrView = {
    ...
    update: function(score){
        var catchPhrase;
        if(score>0) catchPhrase = "Flirty!";
            else if(score == 0) catchPhrase = "Neutral!";
                else {catchPhrase = "Friendly :("}
        $(".answer").text("Score: "+score+" - "+catchPhrase);
        $(".answer").show();
